...
const TubeMapVisualization = ({
  ...
  useEffect(() => {
    ...
    // Store port positions for scroll detection
    const positions = {};
    Object.entries(processedPorts).forEach(([portName, portData]) => {
      positions[portName] = {
        startY: portData.startY,
        endY: portData.endY,
        midY: (portData.startY + portData.endY) / 2
      };
    });
    ...
    Object.entries(processedPorts).forEach(([portName, portData]) => {
      const portGroup = svg.append('g')
        ...
        .attr('data-port', portName); // Add data-port attribute for scroll detection
      ...
      sortedCargoEntries.forEach(([cargoId, cargoInfo]) => {
        ...
        const hasBreakout = eventsForCargo.some(e => 
          e.cargoPositions[cargoId] && e.cargoPositions[cargoId].isInOperations
        );
        ...
        // Use tank designation for CSS classes
        const cargoHoldClass = cargoInfo.tankDesignation ? 
          `cargo-${cargoInfo.tankDesignation.replace(/[^a-zA-Z0-9]/g, '-')}` : 
          `cargo-${cargoId}`;
      ...
      portData.events.forEach(event => {
        ...
        // Add charterer labels for multi-charterer operations at Port Qasim
        if (portName === 'Port Qasim' && event.type === 'individual' && event.cargo) {
          const cargoInfo = cargoTypes[event.cargo];
          const position = event.cargoPositions[event.cargo];
          ...
          if (cargoInfo && position && position.isInOperations) {
            ...
            const labelGroup = stationGroup.append('g')
              .attr('class', 'charterer-label-group');
            
            // Background rectangle for the label
            const labelText = chartererLabel;
            ...
            labelGroup.append('text')
              .attr('x', position.x)
              .attr('y', event.y - 30 + labelHeight / 2 + labelFontSize / 3)
        ...
        // Helper function to replace SOF identifiers with tank designations in event text
        const replaceSOFWithCargoHold = (eventText, cargoId) => {
          if (!cargoId || !cargoTypes[cargoId]) return eventText;
          
          const tankDesignation = cargoTypes[cargoId].tankDesignation;
          // Replace cargo ID with the actual tank designation
          return eventText.replace(new RegExp(`\\b${cargoId}\\b`, 'g'), tankDesignation);
        };
  ...
  useEffect(() => {
    if (!onPortInView || activePort !== 'all') return; // Only detect when viewing all ports
    ...
    const handleScroll = () => {
      ...
      // Find which port is currently in the center of the viewport
      let closestPort = null;
      let closestDistance = Infinity;

      Object.entries(portPositions).forEach(([portName, position]) => {
        // Adjust port positions relative to container
        const adjustedMidY = position.midY;
        const distance = Math.abs(adjustedMidY - viewportCenter);
        
        // Check if port is within viewport
        const isInViewport = adjustedMidY >= containerScrollTop && 
                           adjustedMidY <= (containerScrollTop + containerRect.height);
      ...
      if (closestPort) {
        console.log('Port in view:', closestPort); // Debug log
        onPortInView(closestPort);
      }
    };
